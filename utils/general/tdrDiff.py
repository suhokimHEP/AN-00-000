...
class tdrDiff(object):
    ...
    def __init__(self, docTag, docPath='papers',  revDiff='HEAD~1', verbosity=0, accessType='ssh', outfile=None, revBase='HEAD', logfile=None):
        """
    ...
    def versionCheck(self):
        ...
        if proc:
            (major, minor, *_) = (((proc.stdout.decode('utf-8')).split(' '))[-1]).split('.')
    ...
    def differ(self):
        ...
        os.chdir('export') # need to work in directory with all TeX includes
        ...
        with open(diffile, mode='w') as out:
            try:
                subprocess.run([str(self._latexdiff), '--verbose', '--flatten', docName, str(export0/docName)], stdout=out, stderr=subprocess.PIPE)
            except subprocess.CalledProcessError as e:
...
def main(argv):
    ...
    d = tdrDiff(opts.tag, opts.docPath, opts.revDiff, opts.verbose, opts.accessType, opts.outfile, opts.revBase, opts.logfile)
    d.differ()
